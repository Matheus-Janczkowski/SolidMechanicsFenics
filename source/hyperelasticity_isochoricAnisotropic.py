...
def simulation_engine(parameters, fibers_vectors, dPsi_dI1Model, 
dPsi_dI2Model, ddPsi_ddI1Model, ddPsi_ddI2Model, ddPsi_dI1dI2Model,
dPsi_dI1PseudoModel, dPsi_dI2PseudoModel, dPsi_dI3PseudoModel,
ddPsi_ddI1PseudoModel, ddPsi_ddI2PseudoModel, ddPsi_ddI3PseudoModel,
...
file_displacementName, file_vonMisesName, T_max):
    ...
    # Calculates the isochoric invariants of C
    ...
    # Calculates the isochoric pseudo-invariants

    I1_isoPseudo = dot(as_vector(fibers_vectors[0]),C*as_vector(
    fibers_vectors[0]))

    I2_isoPseudo = dot(as_vector(fibers_vectors[1]),C*as_vector(
    fibers_vectors[1]))

    I3_isoPseudo = dot(as_vector(fibers_vectors[2]),C*as_vector(
    fibers_vectors[2]))
    
	# Defines the tensor products between the fiber vectors
    
    a1xa1 = elastic_tensors.tensor_vectorByItself(as_vector(
    fibers_vectors[0]))
    
    a2xa2 = elastic_tensors.tensor_vectorByItself(as_vector(
    fibers_vectors[1]))
    
    a3xa3 = elastic_tensors.tensor_vectorByItself(as_vector(
    fibers_vectors[2]))
    ...
    dPsi_dI1Iso = dPsi_dI1Model(I1_iso, I2_iso, parameters)

    dPsi_dI2Iso = dPsi_dI2Model(I1_iso, I2_iso, parameters)

    ddPsi_ddI1Iso = ddPsi_ddI1Model(I1_iso, I2_iso, parameters)

    ddPsi_ddI2Iso = ddPsi_ddI2Model(I1_iso, I2_iso, parameters)

    ddPsi_dI1dI2Iso = ddPsi_dI1dI2Model(I1_iso, I2_iso, parameters)

    # Defines the first and second derivatives of the anisotropic part 
    # of the energy function

    dPsi_dI1IsoPseudo = dPsi_dI1PseudoModel(I1_isoPseudo, I2_isoPseudo,
    I3_isoPseudo, parameters_aniso)

    dPsi_dI2IsoPseudo = dPsi_dI2PseudoModel(I1_isoPseudo, I2_isoPseudo,
    I3_isoPseudo, parameters_aniso)

    dPsi_dI3IsoPseudo = dPsi_dI3PseudoModel(I1_isoPseudo, I2_isoPseudo,
    I3_isoPseudo, parameters_aniso)

    ddPsi_ddI1IsoPseudo = ddPsi_ddI1PseudoModel(I1_isoPseudo, 
    I2_isoPseudo, I3_isoPseudo, parameters_aniso)

    ddPsi_ddI2IsoPseudo = ddPsi_ddI2PseudoModel(I1_isoPseudo, 
    I2_isoPseudo, I3_isoPseudo, parameters_aniso)

    ddPsi_ddI3IsoPseudo = ddPsi_ddI3PseudoModel(I1_isoPseudo, 
    I2_isoPseudo, I3_isoPseudo, parameters_aniso)

    ddPsi_dI1dI2IsoPseudo = ddPsi_dI1dI2PseudoModel(I1_isoPseudo, 
    I2_isoPseudo, I3_isoPseudo, parameters_aniso)

    ddPsi_dI1dI3IsoPseudo = ddPsi_dI1dI3PseudoModel(I1_isoPseudo, 
    I2_isoPseudo, I3_isoPseudo, parameters_aniso)

    ddPsi_dI2dI3IsoPseudo = ddPsi_dI2dI3PseudoModel(I1_isoPseudo, 
    I2_isoPseudo, I3_isoPseudo, parameters_aniso)
    ...
    S = (2*((J_23*(dPsi_dI1Iso+(I1_iso*dPsi_dI2Iso))*I)-(J_23*J_23*
    dPsi_dI2Iso*C)+((k_vol-((1/3)*((I1_iso*dPsi_dI1Iso)+(2*I2_iso*
    dPsi_dI2Iso)+(I1_isoPseudo*dPsi_dI1IsoPseudo)+(I2_isoPseudo*
    dPsi_dI2IsoPseudo)+(I3_isoPseudo*dPsi_dI3IsoPseudo))))*inv(C))+(J_23
    *((dPsi_dI1IsoPseudo*a1xa1)+(dPsi_dI2IsoPseudo*a2xa2)+(
    dPsi_dI3IsoPseudo*a3xa3)))))
    ...
    C_elastic = elastic_tensors.C_hyperIsochoricAnisotropic(C, I1_iso,
    I2_iso, k_vol, J, dkvol_dJ, dPsi_dI1Iso, dPsi_dI2Iso, ddPsi_ddI1Iso, 
    ddPsi_ddI2Iso, ddPsi_dI1dI2Iso, dimension, I1_isoPseudo,
    I2_isoPseudo, I3_isoPseudo, dPsi_dI1IsoPseudo, dPsi_dI2IsoPseudo,
    dPsi_dI3IsoPseudo, ddPsi_ddI1IsoPseudo, ddPsi_ddI2IsoPseudo, 
    ddPsi_ddI3IsoPseudo, ddPsi_dI1dI2IsoPseudo, ddPsi_dI1dI3IsoPseudo,
    ddPsi_dI2dI3IsoPseudo, a1xa1, a2xa2, a3xa3)
